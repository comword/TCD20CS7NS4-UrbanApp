...
import org.gtdev.apps.sensinglight.utils.*
...
//            maxWaitTime = TimeUnit.MINUTES.toMillis(1)
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult?) {
                ...
                if (locationResult?.lastLocation != null) {
                    ...
                    saveRecord(locationResult.lastLocation)
                } else {
    ...
    fun saveRecord(location: Location) {
        GlobalScope.launch {
            ...
            val locId = appDatabase!!.locationRecordDao().insert(locEnt)
            val recEnt = RecordEntity(locId)
            appDatabase!!.recordEntityDao().insert(recEnt)

            // Upload
            val key = database.child("records").push().key
            val locationValues = locEnt.toMap(locId)
            val recValues = recEnt.toMap()

            val childUpdates = hashMapOf<String, Any>(
                    "/real-locations/$key" to locationValues,
                    "/real-records/$key" to recValues,
                    "/real-latest/location" to locationValues
            )
    ...
    private fun generateNotification(location: Location?): Notification {
        ...
        return notificationCompatBuilder
            ...
            .setSound(null)
            .addAction(
                R.drawable.ic_launch_new_24, getString(R.string.launch_activity),
    ...
    companion object {
        ...
        private const val PACKAGE_NAME = "org.gtdev.apps.sensinglight"
